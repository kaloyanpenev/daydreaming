...
GameLoop::GameLoop(int _screenWidth, int _screenHeight)
{
...
void GameLoop::RunGame()
{
	...
	else
	{
		...
		//create timer object, explicit type definition once to increase readability for the rest of the initializations, will use "auto" for the rest because the type can be inferred
		std::unique_ptr<Timer> time = std::make_unique<Timer>();
		//create background object
		auto background = std::make_unique<Sprite>(window, renderer, "assets/Background.png", 0, 0, 1280, 720);
		...
		auto scoreText = std::make_unique<TextSprite>(window, renderer, "assets/IndieFlower.ttf", 10, 10, 256, 64, 64, std::string("Score:"), C_YELLOW); 
		...
		while (!exitFlag)
		{
			...
			if (time->getDeltaTime() >= 1.0f / FRAME_RATE) //limit framerate
			{
				...
				scoreText->UpdateFont({ "Score: " + currentScore });
				scoreText->Draw();
				...
				for (auto bullet_itr = bulletList.begin(); bullet_itr != bulletList.end();) //for every bullet in bulletlist
				{
					...
					for (auto enemy_itr = enemies.begin(); enemy_itr != enemies.end();) // OUTER LOOP(for every bullet) checks against INNER LOOP(for every enemy)
					{
						...
						{
							Mix_PlayChannel(-1, deathSFX, 0); //play sound effect
							score += 300; //add 100 scorepoints
...
///Some parts of this function were written by LazyFoo. Available at: http://lazyfoo.net/tutorials/SDL/
bool GameLoop::InitializeSDL()
{
	...
	//load music
	musicTrack = Mix_LoadMUS("assets/music.mp3");

	if (musicTrack == nullptr)
	{
		std::cout << "couldnt load music track" << Mix_GetError();
	}
	deathSFX = Mix_LoadWAV("assets/deathsfx.wav");
